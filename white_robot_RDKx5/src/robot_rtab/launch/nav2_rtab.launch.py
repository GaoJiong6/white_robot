...
def generate_launch_description():
    ...
    nav2_dir = get_package_share_directory('my_obot_nav2')

    turn_on_robot = IncludeLaunchDescription(
            PythonLaunchDescriptionSource(os.path.join(bringup_dir, 'launch','turn_on_robot.launch.py')),
    )
    lidar = IncludeLaunchDescription(
            PythonLaunchDescriptionSource(os.path.join(bringup_dir, 'launch', 'lidar.launch.py')),
    )
    camera = IncludeLaunchDescription(
            PythonLaunchDescriptionSource(os.path.join(bringup_dir, 'launch', 'camera.launch.py')),
    )
    ...
    amcl_config = Path(get_package_share_directory('my_robot_nav2'), 'param', 'param_mini_akm.yaml')
